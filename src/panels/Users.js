...
class Users extends React.Component {
	...
	render() {
		...
		let user_list = [];
		users.forEach((item) => {
			user_list.push(<Div key={item.place}><Link href={"https://vk.com/id" + item.vk_id}>{item.place + ". " + item._vk.first_name + " " + item._vk.last_name + " Баланс: " + item.balance + ". Уровень: " + item.level}</Link></Div>);
		});
		return (
			<Panel id={id}>
				<PanelHeader 
